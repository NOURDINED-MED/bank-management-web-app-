...
export async function POST(request: NextRequest) {
  try {
    ...
    const { tellerId, senderAccountNumber, recipientAccountNumber, amount, description } = body

    // Validate required fields
    if (!tellerId || !senderAccountNumber || !recipientAccountNumber || !amount) {
      return NextResponse.json(
        { error: 'Missing required fields: tellerId, senderAccountNumber, recipientAccountNumber, and amount are required' },
        { status: 400 }
      )
    }
