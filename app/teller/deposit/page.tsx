...
function DepositPageContent() {
  ...
  const searchCustomer = async () => {
    ...
    try {
      const response = await fetch(`/api/teller/search-customer?q=${encodeURIComponent(accountNumber)}`)
      const data = await response.json()
      ...
      // Find exact match by account number
      const foundCustomer = data.customers?.find((c: any) => 
        c.accountNumber?.toLowerCase() === accountNumber.toLowerCase().trim()
      )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/teller/process-deposit', {
        method: 'POST',
        headers: {
