...
function AdminTransferPageContent() {
  ...
  const searchSenderWithAccount = async (accountNum: string) => {
    if (!accountNum.trim() || !user?.id) return
    setSearchingSender(true)
    setError("")
    setSender(null)

    try {
      const response = await fetch(`/api/admin/customers?userId=${user.id}&search=${encodeURIComponent(accountNum)}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/admin/process-transfer', {
        method: 'POST',
        headers: {
      ...
      setTransactionRef(data.referenceNumber || data.transaction?.reference_number || '')
      setSuccess(true)
            ...
            <div className="max-w-2xl mx-auto">
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Redirecting to customer management...</p>
                </CardContent>
              </Card>
            </div>
          </main>
  ...
  return (
    <ProtectedRoute allowedRoles={["admin"]}>
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="max-w-2xl mx-auto">
            <Link href="/admin/customers" className="text-sm text-muted-foreground hover:text-foreground mb-4 inline-block">
              ← Back to Customers
            </Link>
